...
- **起動時の表示**: MIDI からフェーダー／ノブの現在位置を OS が直接読み取ることはできません。アプリは**前回終了時までに MIDI または左パネルで確定した 0…1 の値**を復元します。実機が接続直後に CC を送る場合は、その受信で上書きされます。合わないときは一度フェーダー／ノブを動かしてください。
